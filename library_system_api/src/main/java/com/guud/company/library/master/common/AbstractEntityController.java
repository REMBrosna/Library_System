...
public abstract class AbstractEntityController {
    private static final Logger log = Logger.getLogger(AbstractEntityController.class);
    ...
    protected boolean isActionValid(String action) {
        AbstractEntityController.EntityStatusAction[] actions = AbstractEntityController.EntityStatusAction.values();
        AbstractEntityController.EntityStatusAction[] var6 = actions;
        int var5 = actions.length;

        for (int var4 = 0; var4 < var5; ++var4) {
            AbstractEntityController.EntityStatusAction a = var6[var4];
    ...
    protected Optional<Object> createEntityProxy(String entity, String object) throws Exception {
        ...
        try {
            ...
            } else if (!MasterServiceEntity.isExisting(entity).isPresent()) {
                throw new Exception("entity not mapped: " + entity);
            } else {
                AppUser principal = principalService.getAccountPrincipal();
                if (principal == null) {
                    throw new Exception("principal null");
                } else {
                    Class<?> entityClass = Class.forName(MasterServiceEntity.getMasterServiceEntityByEntityName(entity).getEntityDTOs());
                    Object dto = this.objectMapper.readValue(object, entityClass);
                    AbstractDTO<?, ?> entityDto = (AbstractDTO) dto;
                    Object bean = this.applicationContext.getBean(MasterServiceEntity.getMasterServiceEntityByEntityName(entity).getEntityServices());
    ...
    protected Optional<Object> updateEntityStatusProxy(String object, String entity, String id, String action) throws Exception {
        ...
        try {
            if (StringUtils.isEmpty(entity)) {
                ...
                char status = 'A';
                if (action.equals(AbstractEntityController.EntityStatusAction.INACTIVATE.getAction())) {
                    status = 'I';
                }
                ...
                if (principal == null) {
                    ...
                    AbstractDTO<?, ?> entityDto = (AbstractDTO) dto;
                    log.debug(entityDto.toJson());
                    Object bean = this.applicationContext.getBean((String) MasterServiceEntity.getMasterServiceEntityByEntityName(entity).getEntityServices());
                    IEntityService<?, ?, ?> service = (IEntityService) bean;
        ...
        try {
            if (StringUtils.isEmpty(entity)) {
                ...
                if (!filterRequest.isValid()) {
                    ...
                    filterResponse.setAaData((ArrayList) entities);
