...
const DistrictFormDetails = () => {

    let { viewType, districtId } = useParams();
    ...
    useEffect(() => {
        setLoading(false);
        if (viewType !== "new") {
            sendRequest("api/v1/library/mst/entity/district/" + districtId, "getDistrict", "get", {});
        }
    }, [districtId, viewType]);
            ...
            switch (urlId) {
                ...
                case "saveDistrict":
                    setSnackBarOptions({
                        ...snackBarOptions,
                        success: true,
                        successMsg: t("masters:district.msg.successMsg"),
                        redirectPath: "/master/district/list",
                    });
                    break;
                case "updateDistrict":
                case "deActive":
                case "activate":
                    setSnackBarOptions({
                        ...snackBarOptions,
                        success: true,
                        successMsg: t("masters:district.msg.successOperation"),
                        redirectPath: "/master/district/list",
                    });
                    break;
                default:
    ...
    const handleSubmit = async (values) => {
        ...
        switch (viewType) {
            case "new":
                sendRequest("api/v1/library/mst/entity/district", "saveDistrict", "post", {
                    ...inputData,
                    recStatus: "A",
                });
                break;
            ...
            case "edit":
                sendRequest("api/v1/library/mst/entity/district/" + districtId, "updateDistrict", "put", {
                    ...inputData,
                });
                break;
            default:
    ...
    const handleActiveChange = (e) => {
        setTimeout(
            () =>
                sendRequest(
                    "api/v1/library/mst/entity/district/" + districtId + "/activate",
                    "activate",
                    "put",
                    inputData
                ),
    ...
    const handleDeActiveChange = (e) => {
        setTimeout(
            () => sendRequest("api/v1/library/mst/entity/district/" + districtId, "deActive", "delete", {}),
            1000
        );
    };
    ...
    let bcLabel = t("masters:district.details.breadCrumbs.sub.edit");
    if (viewType) {
        switch (viewType) {
            case "view":
                bcLabel = t("masters:district.details.breadCrumbs.sub.view");
                formButtons = (
                    <C1FormButtons
                        options={{
                ...
                break;
            case "edit":
                bcLabel = t("masters:district.details.breadCrumbs.sub.edit");
                formButtons = (
                    <C1FormButtons
                        options={{
                            back: { show: true, eventHandler: () => history.goBack() },
                ...
                break;
            case "new":
                bcLabel = t("masters:district.details.breadCrumbs.sub.new");
                break;
            default:
                break;
        }
    ...
    return loading ? (
        ...
        <React.Fragment>
            <C1FormDetailsPanel
                breadcrumbs={[
                    { name: t("masters:district.details.breadCrumbs.main"), path: "/master/district/list" },
                    { name: bcLabel },
                ]}
                title={bcLabel}
                    ...
                    <Grid container spacing={3}>
                        <Grid item xs={12}>
                            <Paper className="p-3">
                                ...
                                {tabIndex === 1 && (
                                    <C1Propertiestab
                                        dtCreated={inputData.disDtCreate}
                                        usrCreated={inputData.disUidCreate}
                                        dtLupd={inputData.disDtLupd}
                                        usrLupd={inputData.disUidLupd}
                                    />
                                )}
                                {tabIndex === 2 && <C1AuditTab filterId={districtId} />}
                            </Paper>
                        </Grid>
                    </Grid>
                )}
...
export default withErrorHandler(DistrictFormDetails);
